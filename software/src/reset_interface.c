...
static uint16_t resetd_open(
	uint8_t __unused rhport,
...
static bool resetd_control_xfer_cb(
	uint8_t __unused rhport,
	uint8_t stage,
	tusb_control_request_t const * request)
{
	...
	if (request->wIndex == itf_num)
	{
		...
		if (request->bRequest == RESET_REQUEST_BOOTSEL) {
			...
			reset_usb_boot(
				gpio_mask,
				(request->wValue & 0x7f) | PICO_STDIO_USB_RESET_BOOTSEL_INTERFACE_DISABLE_MASK);
